...
Model of 8 bins with 8 resident species and 24 migrant species (M3,M11,M19&M27 were removed due to crashing the model):
```{r}
...
matplot(LV.out[,1],LV.out[,2:33],type="l",ylab="Population Size",xlab="Time (yrs)",main="Taiwan populations for 8 bins")
```

Bipartite graph of the population sizes:
```{r}
...
par(mar = c(0.1, 0.1, 0.7, 0.1))  

l <- layout.bipartite(g)
V(g)$color <- V(g)$type
V(g)$color=gsub("FALSE","red",V(g)$color)
V(g)$color=gsub("TRUE","blue",V(g)$color)
plot(g, edge.color="green3",edge.width=E(g)$weight/200,vertex.size=3*log(c(Ks1,Ks2,Ks3,Ks4,Ks5,Ks6,Ks7,Ks8,Kw1,Kw2,Kw3,Kw4,Kw5,Kw6,Kw7,Kw8)), layout=l[, c(2,1)],main="Taiwan 8 bins")
```
The formatting needs tweaking a bit but overall this works.
